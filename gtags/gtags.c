...
//KALS
#define OPT_DONT_FOLLOW		135
	/* flag value */
	{"accept-dotfiles", no_argument, NULL, OPT_ACCEPT_DOTFILES},
	{"debug", no_argument, &debug, 1},
	{"explain", no_argument, &explain, 1},
#ifdef USE_SQLITE3
	{"nosqlite3", no_argument, &use_sqlite3, 0},
#endif
	{"skip-unreadable", no_argument, NULL, OPT_SKIP_UNREADABLE},
//KALS
	{"xref", no_argument, NULL, OPT_DONT_FOLLOW},
	{"dont-follow-symlink", no_argument, NULL, OPT_DONT_FOLLOW},
...
//KALS
extern int dont_follow_symlink;
...
main(int argc, char **argv)
{
	...
	while ((optchar = getopt_long(argc, argv, "cd:f:iIn:oOqvwsex", long_options, &option_index)) != EOF) {
		switch (optchar) {
		case 0:
			/* already flags set */
			break;
...
//KALS
static void
put_syms(int type, const char *tag, int lno, const char *path, const char *line_image, void *arg, const char* xtag)
{
	...
	//KALS
	if(type == PARSER_REF_SYM)
	{
		/*printf("[ TAG ] caller:%s, tag:%s\n", xtag, tag);*/
	}
	gtags_put_using(gtop, tag, lno, data->fid, line_image, xtag);
}
/**
...
updatetags(const char *dbpath, const char *root, IDSET *deleteset, STRBUF *addlist)
{
	...
	for (path = start; path < end; path += strlen(path) + 1) {
		...
		//KALS
		gtags_flush(&data, data.gtop[GTAGS], data.fid);
		if (data.gtop[GRTAGS] != NULL)
		{
			//KALS
			gtags_flush(&data, data.gtop[GRTAGS], data.fid);
		}
	}
	parser_exit();
