...
char * dbop3_quote_table(char *string) ;
...
//KALS
extern int vflag;
char* trim_single_quote(const char *cs)
{
	char *s = (char *)cs;
	while(*s)
	{
...
char * dbop3_get_key_rowid(DBOP *dbop, const char *name, STRBUF *sbuf) ;
char * dbop3_get_rowid_key(DBOP *dbop, const char *name, STRBUF *sbuf) ;
/**
 * dbop_get: get data by a key.
 *
 *	@param[in]	dbop	descripter
...
typedef struct lastrec
{
	char rowid[256];
	char key[1024];
}LASTREC;

char * dbop_get_key_rowid(DBOP *dbop, const char *name, STRBUF *sbuf)
{
	DB *db = dbop->db;
	int status;
	char *key = "0";
	static LASTREC lastrec={0};

	if(strcmp(name, lastrec.key) == 0)
	{
		return lastrec.rowid;
	}


#ifdef USE_SQLITE3
	if (dbop->openflags & DBOP_SQLITE3)
		key= dbop3_get_key_rowid(dbop, name, sbuf);
#endif

	strcpy(lastrec.key, name);
	strcpy(lastrec.rowid, key);
...
char * dbop_get_rowid_key(DBOP *dbop, const char *name, STRBUF *sbuf)
{
	DB *db = dbop->db;
	int status;
	char *key = "<global>";
...
#ifdef USE_SQLITE3
	if (dbop->openflags & DBOP_SQLITE3)
		key =  dbop3_get_rowid_key(dbop, name, sbuf);
#endif
...
//KALS
		char *file = rindex(path, '/');
		if( file != NULL && strcmp(file, "GTAGS") )
		{

			/* drop table */
			strbuf_clear(sql);
			strbuf_puts(sql, "drop table sym");
			...
			/* create table */
			strbuf_clear(sql);
			strbuf_puts(sql, "create table sym");
			strbuf_puts(sql, " (key text");
			if (!(flags & DBOP_DUP))
				strbuf_puts(sql, ", primary key(key)");
			strbuf_putc(sql, ')');
	...
	*/
	...
	if(vflag)
			printf("[ QUERY ]%d: %s\n",__LINE__,  "pragma journal_mode=memory" );
...
dbop3_get(DBOP *dbop, const char *name) {
	...
	strbuf_sprintf(sql, "select dat, extra from %s where key = '%s' limit 1",
			dbop->tblname, dbop3_quote(name) ); 
	dbop->lastdat = NULL;
	dbop->lastsize = 0;
	dbop->lastflag = NULL;

	if(vflag)
		printf("[ QUERY ]%d,%s: %s\n",__LINE__, dbop->dbname, strbuf_value(sql) );
	...
	strbuf_release_tempbuf(sql);
	if(vflag)
		printf("[ RESULT ] %d, %s\n", __LINE__, dbop->lastdat?dbop->lastdat:"empty");
	return dbop->lastdat;
}
...
static int key_rowid_single_select_callback(void *v, int argc, char **argv, char **colname) {
	STRBUF *sb = (STRBUF*)v;
...
char * dbop3_get_key_rowid(DBOP *dbop, const char *name, STRBUF *sbuf) {
	...
	strbuf_sprintf(sql, "select rowid from sym where key = '%s' limit 1",
			trim_single_quote(name) ); 

	if(vflag)
		printf("[ QUERY ]%d: %s\n",__LINE__,  strbuf_value(sql) );

	rc = sqlite3_exec(dbop->db3, strbuf_value(sql), key_rowid_single_select_callback, sbuf, &errmsg);
       	if (rc != SQLITE_OK) {
		sqlite3_close(dbop->db3);
		die("dbop3_get failed: %s", errmsg);
	}
	...
	if( strbuf_null(sbuf) )
	{
		strbuf_clear(sql);
#if 0
		strbuf_sprintf(sql, "insert into sym values ('%s')", name);
		...
		strbuf_sprintf(sbuf, "%d", sqlite3_last_insert_rowid(dbop->db3) );
#else
		if (dbop->stmt_put4 == NULL) {
			strbuf_sprintf(sql, "insert into sym values (?)");
			if(vflag)
				printf("[ QUERY ]%d,%s: %s\n",__LINE__, "sym",  strbuf_value(sql) );
			rc = sqlite3_prepare_v2(dbop->db3, strbuf_value(sql), -1, &dbop->stmt_put4, NULL);
			if (rc != SQLITE_OK) {
				die("dbop3_put prepare failed. (rc = %d, sql = %s)", rc, strbuf_value(sql));
			}
		}

		rc = sqlite3_bind_text(dbop->stmt_put4, 1, name, -1, SQLITE_STATIC);
		if (rc != SQLITE_OK) {
			die("dbop3_put failed. (rc = %d)", rc);
		}
		rc = sqlite3_step(dbop->stmt_put4);
		if (rc != SQLITE_DONE) {
			die("dbop3_put failed. (rc = %d)", rc);
		}
		rc = sqlite3_reset(dbop->stmt_put4);
		if (rc != SQLITE_OK) {
	...
	strbuf_release_tempbuf(sql);
	if(vflag)
		printf("[ KEY RESULT ] %d, %s\n", __LINE__, strbuf_value(sbuf));
	return strbuf_value(sbuf);
}
...
char * dbop3_get_rowid_key(DBOP *dbop, const char *name, STRBUF *sbuf) {
	...
	strbuf_sprintf(sql, "select key from sym where rowid = %s limit 1", name ); 
...
dbop3_close(DBOP *dbop) {
	...
	if (dbop->mode == 1 && dbop->openflags & DBOP_DUP) {
		...
		//KALS
		char *file = rindex(dbop->dbname, '/');
		if( file != NULL && strcmp(file, "GTAGS") )
		{
			strbuf_clear(sql);
			strbuf_puts(sql, "create index sym_i on sym(key)");
	...
	//KALS
	if (dbop->stmt_put4) {
		rc = sqlite3_finalize(dbop->stmt_put4);
		if (rc != SQLITE_OK)
			die("dbop3_finalize failed. (rc = %d)", rc);
		dbop->stmt_put4 = NULL;
	}
