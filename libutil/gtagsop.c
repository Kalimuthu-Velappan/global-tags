...
//KALS
static void flush_pool(struct put_func_data *, GTOP *, const char *);
...
compare_lineno(const void *s1, const void *s2)
{
	return ((REF *)s1)->line - ((REF *)s2)->line;
}
#endif
...
gtags_open(const char *dbpath, const char *root, int db, int mode, int flags)
{
	...
	if (gtop->mode == GTAGS_CREATE) {
		...
		if(flags & GTAGS_COMPACTXREF)
			gtop->format |= GTAGS_COMPACTXREF;
		...
		if (gtop->format & GTAGS_COMPACTXREF)
			dbop_putoption(gtop->dbop, COMPACTXREF, NULL);
		...
		if (dbop_getoption(gtop->dbop, COMPACTXREF) != NULL)
			gtop->format |= GTAGS_COMPACTXREF;
	}
...
void
gtags_put_using(GTOP *gtop, const char *tag, int lno, const char *fid, const char *img, const char* xtag)
{
	const char *key;
	...
	if (gtop->format & GTAGS_COMPACT) {
		...
		*(int *)varray_append((VARRAY *)entry->value) = lno;
		return;
			...
			//KALS
			if (entry->value == NULL)
				entry->value = varray_open(sizeof(REF), 50);
			REF *ref = (REF *)varray_append((VARRAY *)entry->value);
			memset(ref, 0, sizeof(REF));
			ref->line = lno;
			strncpy(ref->func, xtag, sizeof(ref->func));
			return;
#endif			
		}
	}
	...
	//KALS
	if(xtag == NULL)xtag = "<GBL>";
	strbuf_putc(gtop->sb, ' ');
	strbuf_puts(gtop->sb, (gtop->format & GTAGS_COMPNAME) ? compress(xtag, key) : xtag);
 ...
 */
void
gtags_flush(void *table, GTOP *gtop, const char *fid)
{
	if (gtop->format & GTAGS_COMPACT) {
		flush_pool(table, gtop, fid);
 ...
 */
static void
flush_pool(struct put_func_data *table, GTOP *gtop, const char *s_fid)
{
	struct sh_entry *entry;
	int header_offset;
	int i, last;
...
#else
		REF *lno_array = varray_assign(vb, 0, 0);
#endif
...
//KALS
		...
		qsort(lno_array, vb->length, sizeof(REF), compare_lineno); 
#endif
...
//KALS
			 ...
			 */
			last = 0;			/* line 0 doesn't exist */
			STRBUF *sbuf = strbuf_open(MAXBUFLEN);
			for (i = 0; i < vb->length; i++) {
#ifndef GRTAGS_EXTENDED
				int n = lno_array[i];
				if (n == last)
				...
				if(gtop->format & GTAGS_COMPACTXREF)
				{
					...
					char buf[256]="1";
					char *tag = NULL;
					GTOP *tgtop = table->gtop[GTAGS];
					if (tgtop != NULL &&  ((tag = dbop_get_key_rowid(tgtop->dbop, n->func, sbuf)) != NULL ))
					{
						/*printf(" GRTAG: %s, TAG: %s, ROWID:%s\n", n->func, n->func, tag ); */
						strbuf_puts(gtop->sb, tag);
					}
				}else{
					strbuf_puts(gtop->sb, n->func);
				}
