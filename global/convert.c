...
convert_put(CONVERT *cv, const char *ctags_x)
{
	...
	case FORMAT_GTAGS:
		...
		if(xtag != NULL){
			fputc('\t', cv->op);
			fprintf(cv->op, "%s", xtag);
		}
		fputc('\t', cv->op);
		fputs(rest, cv->op);
		break;
	case FORMAT_GTAGS_X:
		fprintf(cv->op, "%-30s", convert_pathname(cv, path) );

		len = strlen(convert_pathname(cv, path));
		if(len > 30)
		{
			len = (10 - (len%10) );
		}else{
		...
		fprintf(cv->op, "|%4d| ", lineno);
		if( rest != NULL)
			fprintf(cv->op, " %s", trim(rest));
		break;
		//KALS
	case FORMAT_GTAGS_SCOPE:
		fprintf(cv->op, "%-30s", convert_pathname(cv, path) );
...
//KALS
void
convert_put_using(CONVERT *cv, const char *tag, const char *path, int lineno, char *rest, const char *fid, char *xtag)
{
	int len =0;
	if (rest == NULL)
		rest = "";	/* for safety */
