...
static int put_compact_format(void *, CONVERT *, GTP *, const char *, int);
 ...
 */
int
output_with_formatting(void *data, CONVERT *cv, GTP *gtp, const char *root, int flags)
{
	int count = 0;
	...
	if (format == FORMAT_PATH) {
		...
		count += put_compact_format(data, cv, gtp, root, flags);
	} else {
 ...
 */
static int
put_compact_format(void *data, CONVERT *cv, GTP *gtp, const char *root, int flags)
{
	struct searchfile_data *table = (struct searchfile_data*)data;
	STATIC_STRBUF(ib);
	int count = 0;
	...
	*p++ = '\0';			/* b */
	...
	if (flags & GTAGS_COMPLINE) {
		...
		while (*p || cont > 0) {
			...
			//KALS
			convert_put_using(cv, tagname, gtp->path, n, src, fid, NULL);
			count++;
			last_lineno = last = n;
		}
	} else {
		...
		while (*p) {
			...
			char *tag = "<global>";
			if(cv->format  == FORMAT_GTAGS_SCOPE )
			{
				if( table->gtop != NULL && table->gtop->format & GTAGS_COMPACTXREF)
				{
					strbuf_clear(sbuf);
					if (strcmp(xtag, "") != 0 && ((tag = dbop_get_rowid_key(table->gtop->dbop, xtag, sbuf)) != NULL ))
					{
						/*printf(" GRTAG: %s, ROWID:%s\n", xtag, tag ); */
					}
				}else{
					tag = xtag;
				}
			...
			//KALS
			convert_put_using(cv, tagname, gtp->path, n, src, fid, tag);
			count++;
			last_lineno = n;
		}
		strbuf_close(sbuf);
...
put_standard_format(CONVERT *cv, GTP *gtp, int flags)
{
	...
	//KALS
	char xtagbuf[IDENTLEN], *xtag = NULL;
	...
	*p++ = '\0';			/* b */
	...
	if (nosource) {
		...
		while (*p != ' ')
			...
			*p = '\0';			/* c */
			if (flags & GTAGS_COMPNAME) {
				strlimcpy(xtagbuf, (char *)uncompress(xtag, gtp->tag), sizeof(xtagbuf));
				xtag = xtagbuf;
			}
	...
	//KALS
	convert_put_using(cv, tagname, gtp->path, gtp->lineno, image, fid, xtag);
}
