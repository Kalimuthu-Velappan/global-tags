...
main(int argc, char **argv)
{
	...
	while ((optchar = getopt_long(argc, argv, "acde:EifFgGIlL:MnNoOpPqrsS:tTuvVx", long_options, &option_index)) != EOF) {
		...
		case OPT_RESULT:
			//KALS
			if (!strcmp(optarg, "gtags-scope"))
				format = FORMAT_GTAGS_SCOPE;
			else if (!strcmp(optarg, "gtags-x"))
				format = FORMAT_GTAGS_X;
			else if (!strcmp(optarg, "gtags"))
				format = FORMAT_GTAGS;

			else if (!strcmp(optarg, "ctags-x"))
	...
	while ((grep = strbuf_fgets(ib, ip, STRBUF_NOCRLF)) != NULL) {
		...
		*q = '\0'; 
		...
		default:
			 ...
			 */
			//KALS
			convert_put_using(cv, pattern, path, linenum, p, NULL, NULL);
			break;
		}
	}
	if (pclose(ip) != 0)
				...
				if ((!Vflag && result == 0) || (Vflag && result != 0)) {
					...
					if (format == FORMAT_PATH) {
						...
						convert_put_using(cv, pattern, path, linenum, buffer,
							(user_specified) ? NULL : gp->dbop->lastdat, NULL);
					}
				}
			}
			fclose(fp);
...
pathlist(const char *pattern, const char *dbpath)
{
	...
	while ((path = gfind_read(gp)) != NULL) {
		...
		if (format == FORMAT_PATH)
			...
			convert_put_using(cv, pattern, path, 1, pathstr, gp->dbop->lastdat, NULL);
		count++;
	}
	gfind_close(gp);
	convert_close(cv);
...
//KALS
static void
put_syms(int type, const char *tag, int lno, const char *path, const char *line_image, void *arg, char *xtag)
{
	...
	//KALS
	convert_put_using(data->cv, tag, path, lno, line_image, data->fid, xtag);
	data->count++;
}
void
...
search(const char *pattern, const char *root, const char *cwd, const char *dbpath, int db)
{
	...
	if (db == GRTAGS ) {
		/*data.dbop = dbop_open(makepath(dbpath, dbname(GTAGS), NULL), 0, 0, 0);*/
		data.gtop = gtags_open(dbpath, root, GTAGS, GTAGS_READ, debug ? GTAGS_DEBUG : 0);
		if (data.gtop == NULL)
			die("%s not found.", dbname(GTAGS));
	} else {
		data.gtop = NULL;
	...
	for (gtp = gtags_first(gtop, pattern, flags); gtp; gtp = gtags_next(gtop)) {
		if (Sflag && !locatestring(gtp->path, localprefix, MATCH_AT_FIRST))
			continue;
		count += output_with_formatting(&data, cv, gtp, root, gtop->format);
	}
	convert_close(cv);
	if (debug)
		gtags_show_statistics(gtop);
